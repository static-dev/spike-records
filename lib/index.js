...
function renderFile (root, obj) {
  return node.call(fs.readFile.bind(fs), path.join(root, obj.file), 'utf8')
    .then((content) => { return JSON.parse(transformRaw(obj, content)) })
}
...
function transformData (data, k) {
  return this.opts[k].transform ? this.opts[k].transform(data) : data
}

/**
  * If the user provided a transformRaw function for a given data source, run the
  * function and return the transformed data. Otherwise, return the data as it
  * exists inititally.
 * @param  {Object} obj - this.opts[k] object
 * @param  {Object} data - data as resolved from user-provided data source
 * @return {Object} Modified or original data
 */
function transformRaw (obj, data) {
  return obj.transformRaw ? obj.transformRaw(data) : data
}
